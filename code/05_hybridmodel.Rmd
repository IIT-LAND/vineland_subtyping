...
libraries('here','ggplot2','gplots','reshape2','gridExtra','ggpubr')
library(dplyr)
...
# load data
data <- read.csv(file.path(result_path,'uscd_4modelcluster_longitudinal.csv'))
...
data$hybrid_cluster = factor(data$hybrid_cluster, levels = c('Rhigh-Vhigh','Rhigh-Vmed','Rhigh-Vlow','Rmed-Vhigh','Rmed-Vmed',
                                                             'Rmed-Vlow','Rlow-Vhigh','Rlow-Vmed','Rlow-Vlow'))
...
for (i in 1:length(var2use)) {
  ...
  print(p_com)
  #save the plot
  ggsave(file.path(plot_path,paste(col2use,'_hybrid.png',sep="")),p_com, width = 6 ,height = 4)
  ...
  write.csv(lme_definitive_2save,file.path(result_path,paste('ANOVA_lme_spagh2_UCSD_hybrid_MELS_',col2use,".csv")))
...
for (i in (1:4)) {
  ...
  for(o in 1:10){
    ...
    write.csv(lm,file.path(result_path,"post_hoc",'MELS_UCSD_hybrid',paste('Post_hoc_ANOVA_lme_spagh2_UCSD_hybrid_',col2use,"_",sub1,"_",sub2,".csv",sep='')))
...
write.csv(ANOVA_post_hoc[2:length(ANOVA_post_hoc$col2use),],file.path(result_path,"Posthoc_NDAR_hybrid.csv")) 
```
...
# plot
ylim_list = list(c(12200,12600),c(11000,11400),c(13000,13400),c(12600,13000))
plot_aic= list()
...
for (i in 1:4){
  ...
  plt = ggplot(data2plot,aes(variable,value,fill=model))+
...
show(aic_plot)    
ggsave(file.path(plot_path,'AIC_plot.png'), aic_plot, width = 6 ,height = 4)
```
...
#     ncol=2,nrow = 2)
#   print(g)
#   #ggsave(file.path(plot_path, paste("MULLEN_",subtype2use,".png",sep='')), g)
# }
