...
spaghettiPlot2 <- function(data2use, x_var, y_var, subgrp_var, cov_vars=NULL, xLabel, yLabel,
                           modelType = "quadratic",
  ...
  if (modelType=="linear") {
    ...
    form2use = as.formula(sprintf("%s ~ %s + I(%s^2) + %s + %s:%s + I(%s^2):%s%s + (1|%s) + (%s|subjectId)",
                                  y_var, x_var, x_var, subgrp_var, x_var, subgrp_var, x_var, subgrp_var, covs2use, "dataset", x_var))
  }# if (modelType=="linear")
  ...
  if (modelType=="linear") {
    ...
    rx_form = as.formula(sprintf("~ 1|%s + %s|subjectId","dataset",x_var))
  } else if (modelType=="quadratic") {
