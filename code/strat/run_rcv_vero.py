...
class RCV:
    ...
    def gridsearch_cv(self, df, n_neigh, na_perc, cl_range, cv_fold, iter_cv=1, save=None):
        ...
        subdomain_feat = [c for c in df.columns if re.search('subdom', c) and not re.search('written', c)] ###subdom 
        domain_feat = [c for c in df.columns if re.search('totalb', c) and not re.search('composite', c)] ### dom
        if save is not None:
        ...
        for k in cv_fold:
            ...
            _msg = f"Training Fold Number {k}"
            print('='*len(_msg))
            print(_msg)
            print('='*len(_msg))
            
            for nap in tqdm(na_perc):
                ...
                for n in n_neigh:
                    ...
                    for p, tup in dict_imp.items():
                        ...
                        #scaled_tr = scaler.fit_transform(tup[0][subdomain_feat]) ### subdom
                        #scaled_ts = scaler.transform(tup[1][subdomain_feat])  ###  subdom
    ...
    def run_rcv(self, df, demo_info, na_perc, n_neigh, cv_fold, cl_range, iter_cv=1, scatter=False, heatmap=False):
        ...
        # subdomain
        for p, tup in dict_imp.items():
            dict_imp[p][0]['sex'] = [demo_info[gui].sex for gui in dict_imp[p][0].index]
            dict_imp[p][0]['phenotype'] = [demo_info[gui].phenotype.replace("'", "") for gui in dict_imp[p][0].index]
            dict_imp[p][0]['race'] = [demo_info[gui].race for gui in dict_imp[p][0].index]
            dict_imp[p][0]['collection_id'] = [dict_tr[p].loc[gui].collection_id for gui in dict_imp[p][0].index]
            dict_imp[p][0]['interview_age'] = [dict_tr[p].loc[gui].interview_age for gui in dict_imp[p][0].index]
...
#domain
        ...
        new_dict_imp = dict_imp #relabel(dict_imp, plot_scatter=False) ### modificato here!!!
        ...
        ##################################### here if you want dist mat
        #_build_distmat(imp_dict=dict_imp)

        return dict_imp,strat_sex,strat_agesexed,X_tr,metric
...
def _build_distmat(imp_dict):
    ...
    feat_dict = {
        ...
        ###################################### change here P1- P2
        'domain': [c for c in imp_dict['P2'][0].columns if re.search('totalb', c) and not re.search('composite', c)]}
