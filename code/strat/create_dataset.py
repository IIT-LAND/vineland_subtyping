...
def prepare_imputation(df, missing_perc=0.35):
    ...
    #df_rid.interview_age = int(df_rid.interview_age)
    #df_rid.interview_age = df_rid.interview_age.astype(str).astype(int)
    #df_rid.insert(0, 'ageyrs_round',0) #round(df_rid.interview_age/12))
    #for sub in df_rid.index:
    #    if  df_rid.loc[sub,'interview_age'] <= int(50):
    #        df_rid.loc[sub,'ageyrs_round'] = round(df_rid.loc[sub,'interview_age']/12)
    #    else:
    #        df_rid.loc[sub,'ageyrs_round'] = int(50)
    df_rid.insert(0, 'ageyrs_round',round(df_rid.interview_age/12))
    age20 = df_rid['ageyrs_round'].between(20,25, inclusive='left')
    age25 = df_rid['ageyrs_round'].between(25,30, inclusive='left')
    age30 = df_rid['ageyrs_round'].between(30,40, inclusive='left')
    age40 = df_rid['ageyrs_round'].between(40,100, inclusive='left')
    #age50 = df_rid['ageyrs_round'].between(50,100, inclusive='left')
    df_rid.loc[age20,'ageyrs_round'] = 20
    df_rid.loc[age25,'ageyrs_round'] = 25
    df_rid.loc[age30,'ageyrs_round'] = 30
    df_rid.loc[age40,'ageyrs_round'] = 40
    #df_rid.loc[age50,'ageyrs_round'] = 50
