...
# set paths
work_dir = '/Users/vmandelli/OneDrive - Fondazione Istituto Italiano Tecnologia/vineland_proj_new'#here()

plot_path = file.path(work_dir,'plots','FINAL_correct')
data_path = file.path(work_dir,'results','FINAL_correct')
results_path = file.path(work_dir,'results','FINAL_correct')
code_path = file.path(work_dir,'code','R')
source(file.path(code_path,"cohens_d.R")) 
```

### chose here if you want to run the following analysis on train or test set
* "tr" = trainset
* "ts" = testset
```{r,warning=FALSE,error=FALSE }
# INSERT HERE IF TRAIN OR TEST
...
# evantually save the plot
#pdf(paste(plot_path,'/',db_type,'_sum_colorixmike.pdf', sep=""))  

# # merge all the plot into a single figure
# g = grid.arrange(
#   plt[[1]],
#   plt[[2]],
#   plt[[3]],
...
```{r,warning=FALSE,error=FALSE}
# read reval output
reval_val = read.csv(file.path(data_path,"val_metric_P14soggettti.csv"))
reval_val$cat = 'validation'
reval_tr = read.csv(file.path(data_path,"train_metric_P14soggettti.csv"))
reval_tr$cat = 'train'
...
data_tr <- read.csv(file.path(data_path,datafile))
#load test
datafile = 'imputed_data_P1_ts.csv'
data_ts <- read.csv(file.path(data_path,datafile))
#merge them
col2use=c("subjectkey","sex",'interview_age')
data = rbind(data_tr,data_ts[,1:11])
data_age = rbind(data_tr[,col2use],data_ts[,col2use])
...
#change cluster label in hihg med and low
for (i in (1:length(data$cluster_domain))) {
  if (data[i, 'cluster_domain'] == '2'){
    data[i,'clust_dom'] = "high"
  }
...
cluster_distributioon = table(data$cluster_domain)
#write.csv(cluster_distributioon,file.path(results_path,'cluster_distribution_NDAR.csv'))
